...
try:
    from redis_integration.embedding_cache import EmbeddingCache
...
class EmbeddingGenerator:
    ...
    def __init__(self, model_name: str = "intfloat/e5-base-v2", device: str = None):
        ...
        if REDIS_CACHE_AVAILABLE:
            try:
                ...
                logger.warning(f"Failed to initialize Redis embedding cache: {e}")
    ...
    def generate_embeddings(self, texts: List[str], batch_size: int = 32) -> np.ndarray:
        ...
        try:
            ...
            if self.cache:
                cached_embeddings = self.cache.get_batch_embeddings(cleaned_texts, self.model_name)
                if cached_embeddings is not None:
                    logger.info(f"Retrieved {len(cleaned_texts)} embeddings from cache")
                    return cached_embeddings
                
                # Check for partial cache hits
                cached_indices, cached_embeds = self.cache.get_partial_batch(cleaned_texts, self.model_name)
                if cached_indices:
                    logger.info(f"Found {len(cached_indices)}/{len(cleaned_texts)} embeddings in cache")
            ...
            # Cache the results
            if self.cache and len(embeddings) > 0:
                # Cache the full batch
                self.cache.set_batch_embeddings(cleaned_texts, self.model_name, embeddings)
                # Also cache individual embeddings for future partial hits
                self.cache.cache_individual_embeddings(cleaned_texts, self.model_name, embeddings)
    ...
    def generate_single_embedding(self, text: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            # Try to get from cache first
            if self.cache:
                cached_embedding = self.cache.get_single_embedding(cleaned_text, self.model_name)
                if cached_embedding is not None:
                    logger.debug(f"Retrieved single embedding from cache")
